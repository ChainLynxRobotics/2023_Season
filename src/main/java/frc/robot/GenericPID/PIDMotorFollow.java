...
import frc.robot.GenericPID.Implementations.DtFullForce;
...
public class PIDMotorFollow {
    ...
    public static void test() {
        ...
        var mc = new MotorControlProfile(ControlLevel.VELOCITY, ControlLevel.ACCELERATION, new DtFullForce(maxf,dt, mass));
    ...
    public static void testCrazy() {
        ...
        while (t < 30) {
            ...
            G.addPoint(t, motorEff/100, false, 1);
            G.addPoint(t, m.p(), false, 0);
            t += /*1/100 - 1/2000 + */ random.nextDouble() / 1000;
        }
    ...
    public double motorOutputNow(double x, double v, double t) {
        ...
        if(debug2) System.out.printf("Realtime: %f PID time: %f\n", t, pid.t());
        if (t < pid.t()) {
            if(debug2) System.out.println("PID time already covered!");
            timesBehind = 0;
            double ret = motor.mEffect(lastEffect, v);
            if (debug && ret > 200) {
            ...
            if (t > pid.t() + dt) {
                ...
                if(debug2) System.out.printf("PID time behind, using catchupfunction to catchup! timebehind %f timesbehind %d dt %f to catch up!\n", timeBehind, timesBehind, newdt);
                double effect = nextControlEffect(x, newdt);
                lastEffect = effect;
                double ret = motor.mEffect(effect, v);
                if (debug && ret > 200) {
                    System.out.printf("bruh we be extremin, eff = %f, v = %f, ret = %f\n", effect, v, ret);
                }
                return ret;
            } else {/*if (t >= pid.t())*/
                if(debug2) System.out.println("PID time on track!");
                timesBehind = 0;
